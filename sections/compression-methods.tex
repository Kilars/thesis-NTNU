...
DP has a worst case runtime of $O(n^{2})$, but is used for its simplicity and efficiency in practice \cite{gudmundsson2009compressing}. Addiotionally \textcite{gudmundsson2009compressing}, created an improved version which can run in $O(n\log(n))$ even in if the path is polygonal and self intersects. There are also other implementations of DP that use spatiotemporal distance measures like SED.
\begin{figure}[h]
